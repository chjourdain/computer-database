...
<c:if test="${ pager.nbPages - pager.currentPage < 2}">
	<c:set var="iteration"
		value="${ pager.nbPages - pager.currentPage + 3}" />
</c:if>
...
<c:choose>
	...
	<c:otherwise>
		<c:forEach var="i" begin="${pager.currentPage -2 }"
			end="${iteration+ pager.currentPage - 3}" step="1">
			<li><l:link pager="${pager}" type="dashboard" page="${i}" name="${i}" /> </li>
		</c:forEach>
	</c:otherwise>
</c:choose>